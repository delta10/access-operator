...
var _ = AfterEach(func() {
	...
	controllerLogs, err := e2eutils.Run(cmd)
	if err == nil {
	...
	eventsOutput, err := e2eutils.Run(cmd)
	if err == nil {
	...
	metricsOutput, err := e2eutils.Run(cmd)
	if err == nil {
	...
	podDescription, err := e2eutils.Run(cmd)
	if err == nil {
...
var _ = Describe("Manager", Serial, Ordered, func() {
	...
	It("should ensure the metrics endpoint is serving metrics", func() {
		...
		token, err := e2eutils.ServiceAccountToken(namespace, serviceAccountName)
		...
		Eventually(func(g Gomega) {
			...
			output, getErr := e2eutils.Run(cmd)
		...
		Eventually(func(g Gomega) {
			cmd = exec.Command("kubectl", "logs", controllerPodName, "-n", namespace)
			output, logErr := e2eutils.Run(cmd)
		...
		Eventually(func(g Gomega) {
			metricsOutput, logErr := e2eutils.GetMetricsOutput(namespace, "curl-metrics")
...
func ensureControllerPodName() string {
	Eventually(func(g Gomega) {
		...
		podOutput, err := e2eutils.Run(cmd)
		g.Expect(err).NotTo(HaveOccurred(), "Failed to retrieve controller-manager pod information")
		podNames := e2eutils.GetNonEmptyLines(podOutput)
