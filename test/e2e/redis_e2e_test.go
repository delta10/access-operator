...
`, resourceName, env.namespace, generatedSecretName, resourceName)
		...
		It("should create a RedisAccess resource and create a Redis ACL user via direct connection details", func() {
			...
			e2eutils.WaitForRedisAuthenticationSuccess(env.backendNamespace, resourceName, password)
		})
		...
		It("should create a RedisAccess resource with direct host/port and secret-referenced credentials", func() {
			...
			secretName, err := e2eutils.CreateRedisConnectionDetailsViaSecret(env.namespace, env.conn)
			...
			err = e2eutils.CreateRedisAccessWithConnectionSecretRef(resourceName, env.namespace, generatedSecret, env.conn, secretName, aclRules)
		...
		It("should create a RedisAccess resource using an existing connection secret in the same namespace", func() {
			...
			err = e2eutils.CreateRedisAccessFromSecretReference(resourceName, env.namespace, generatedSecret, secretName, nil, aclRules)
		...
		It("should reconcile permissions when they're changed in the config", func() {
			...
			err = e2eutils.CreateRedisAccessWithDirectConnection(resourceName, env.namespace, generatedSecret, env.conn, newAclRules)
			Expect(err).NotTo(HaveOccurred(), "Failed to update RedisAccess ACL rules")

			By("verifying the Redis ACL rules are updated")
			e2eutils.WaitForRedisACLRules(env.backendNamespace, env.conn, resourceName, newAclRules)
		...
		It("should reconcile Redis ACL rules when they are manually drifted", func() {
			...
			By("creating a RedisAccess resource")
			err := e2eutils.CreateRedisAccessWithDirectConnection(resourceName, env.namespace, generatedSecret, env.conn, desiredRules)
			Expect(err).NotTo(HaveOccurred(), "Failed to create RedisAccess resource")

			By("waiting for the generated secret and initial Redis ACL rules")
			e2eutils.WaitForSecretField(env.namespace, generatedSecret, "username")
			e2eutils.WaitForRedisACLRules(env.backendNamespace, env.conn, resourceName, desiredRules)

			By("manually drifting the Redis ACL user")
			_, err = e2eutils.RunRedisCLI(env.backendNamespace, env.conn,
				"ACL", "SETUSER", resourceName,
				"reset", "on", ">wrong-password", "~drifted:*", "+get",
			)
			Expect(err).NotTo(HaveOccurred(), "Failed to drift Redis ACL user")

			err = e2eutils.TriggerReconciliation("redisaccess", resourceName, env.namespace)
		...
		It("should update the Redis user's password when the generated secret is rotated by deletion", func() {
			...
			e2eutils.WaitForRedisAuthenticationSuccess(env.backendNamespace, resourceName, oldPassword)
			...
			e2eutils.WaitForRedisAuthenticationSuccess(env.backendNamespace, resourceName, newPassword)
		})
		...
		It("should keep Redis ACL users that are still used as connection usernames by another RedisAccess", func() {
			...
			err := e2eutils.CreateRedisAccessWithDirectConnection(protectedUser, env.namespace, protectedSecret, env.conn, []string{"~*", "+@all"})
			...
			e2eutils.WaitForSecretField(env.namespace, protectedSecret, "username")
			protectedPassword := e2eutils.WaitForDecodedSecretField(env.namespace, protectedSecret, "password")
			e2eutils.WaitForRedisAuthenticationSuccess(env.backendNamespace, protectedUser, protectedPassword)

			By("creating another RedisAccess that uses the protected user for its Redis connection")
			err = e2eutils.CreateRedisAccessWithConnectionSecretRef(dependentResource, env.namespace, dependentSecret, controller.ConnectionDetails{
			...
			e2eutils.WaitForSecretField(env.namespace, dependentSecret, "username")
			e2eutils.WaitForRedisUserState(env.backendNamespace, env.conn, dependentResource, true)

			By("deleting the RedisAccess that owns the connection username")
			err = e2eutils.DeleteRedisAccess(protectedUser, env.namespace)
			...
			e2eutils.WaitForResourceDeleted("redisaccess", protectedUser, env.namespace)
			e2eutils.WaitForRedisUserState(env.backendNamespace, env.conn, protectedUser, true)
			e2eutils.WaitForSecretDeleted(env.namespace, protectedSecret)
		})
	})
})
